                ...
                <div id="page-content">
                    ...
                    <div class="content-header">
                        <div class="header-section">
                            <h1>
                                <i class="gi gi-notes_2"></i>  Family Payment Request Form<br/><small></small>
                            </h1>
                        </div>
                    </div>
                    <ul class="breadcrumb breadcrumb-top">
                        <li><a href="finance.php">Finance</a></li>
                        <li><a href="">  Family Payment Request Form</a></li>
                    </ul>
...
$r=rand();
	...
	$tamount=clean($_POST['amount']);
	$bname=clean($_POST['bname']);
	$bban=clean($_POST['bban']);
	$bbn=clean($_POST['bbn']);
	$pay_c=clean($_POST['pay_c']);
	$chargeto=clean($_POST['chargeto']);
	
	$num_notes=strlen($notes);
	$desc1=clean($_POST['description']);
	$pc=$_SERVER['REMOTE_ADDR'];
	$form_name='Family Payment Request Form';
	$i_p=$_SERVER['REMOTE_ADDR'];
    $entry_time=date('h:i;s');
    $unixtime=time();
    $subject=$user_name.' Generate a Request Req# '. $ref;
    
    $insform=mysql_query("insert into formsdata (id,username,ref,created_date,benif_name,amount,invoice,due_date,description,entry_date,entry_time,Ip,status,formid,benif_bank_ac,benif_bank_name,payc,charg_to,form_name) values
	                                ('','$requester','$ref','$created_date','$bname', '$tamount','$invoice','$due_date','$desc1','$created_date','$entry_time','$i_p','Requested',16,'$bban','$bbn','$pay_c','$chargeto','$form_name')") or die (mysql_error());
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="block">
                                ...
                                <form action="f16-fam-pay-req" method="post" enctype="multipart/form-data" id="testconfirmJQ" name="testconfirmJQ">
 ...
 <table class="table table-bordered" width="969" border="1">
  ...
  <tr>
    <td valign="middle" style="vertical-align:middle;">Benificiary Name</td>
    <td colspan="2"><input type="text" name="bname" id="bname" class="form-control" style="" required/></td>
    <td><span style="vertical-align:middle;">Beneficiary Bank Account Number</span></td>
    <td colspan="2"><input type="text" name="bban" id="bban" class="form-control" style="" required/></td>
   </tr>
  <tr>
    <td width="224" valign="middle" style="vertical-align:middle;">Beneficiary Bank Name</td>
    <td width="150"><label>
      <input type="text" name="bbn" id="bbn" class="form-control" style="" />
    </label></td>
    <td width="138" style="vertical-align:middle;">Payment Currency</td>
    <td width="144"><select name="pay_c" id="pay_c" class="form-control" required>
    ...
    <td width="144"><input type="text" name="amount" id="amount" class="form-control" style="" required/>    </td>
  </tr>
  <tr>
    <td valign="middle" style="vertical-align:middle;">Charge To</td>
    <td colspan="5"><label>
      <select name="chargeto" id="chargeto" class="form-control" required>
        <option value="">Select</option>
        <option>ALIA ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>EISA ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>FADAL ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>FATIMA SULTAN WILDO.O.ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>HAMAD ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>LATIFA ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>MAITHA ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>MARIAM ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>MEERA ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>MUBARAK ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>OMAR ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>SALMA ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>SHAIKA ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>SHAMSA ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>SULTAN ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
        <option>YOUSEF ABDULLA RASHID AL DHAHERI CURRENT A/C</option>
      </select>
    </label></td>
   </tr>
</table>
...
<script>
            $(function(){
                ...
                $('form#testconfirmJQ').submit(function(e){
					...
					$("span#pay_c strong").html($("select#pay_c").val());
					$("span#chargeto strong").html($("select#chargeto").val());
...
<div id="jq-dialog" title="Verify Form ">
  ...
  <tr>
    <td valign="middle" style="vertical-align:middle;">Benificiary Name</td>
    <td colspan="2"><span id="bname"><strong></strong></span></td>
    <td><span style="vertical-align:middle;">Beneficiary Bank Account Number</span></td>
    <td colspan="2"><span id="bban"><strong></strong></span></td>
   </tr>
  <tr>
    <td width="224" valign="middle" style="vertical-align:middle;">Beneficiary Bank Name</td>
    <td width="150"><label>
      <span id="bbn"><strong></strong></span>
    </label></td>
    <td width="138" style="vertical-align:middle;">Payment Currency</td>
    <td width="144"><span id="pay_c"><strong></strong></span></td>
    <td width="129">Amount</td>
    <td width="144"><span id="amount"><strong></strong></span>   </td>
  </tr>
  <tr>
    <td valign="middle" style="vertical-align:middle;">Charge To</td>
    <td colspan="5">
      <span id="chargeto"><strong></strong></span>
    </td>
   </tr>
</table>
